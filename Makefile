	...
	@if [ ! -L w1 ]; then \
	  if [ -d ${KERN_SRC_DIR}/drivers/w1 ]; then \
	    ...
	    if [ "${KERNEL_SRC}x" = "x" ]; then \
	      ...
	      echo "./w1-internal directory). Linux kernel API is not persistent across versions, so it"; \
	      echo "is STRONGLY recommended to set ./w1 symbolic link to \`drivers/w1' subdirectory of"; \
	      echo "the target kernel sources. This way proper w1 headers will be used.\n"; \
